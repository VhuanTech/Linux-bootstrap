...
fix_ubuntu_2004_apt_hook() {
    if [ -f /etc/apt/apt.conf.d/50command-not-found ]; then
        sudo sed -i \
            's/^APT::Update::Post-Invoke-Success/#DISABLED_APT::Update::Post-Invoke-Success/' \
            /etc/apt/apt.conf.d/50command-not-found 2>/dev/null || true
    fi
}
